...
app.get("/api/support/tickets", (req, res) => {
  try {
    ...
    res.json({
      success: true,
      tickets: support.getUserTickets(userId)
    });
  } catch (error) {
    res.status(400).json({
      success: false,
...
app.get("/api/support/tickets/:id", (req, res) => {
  try {
    ...
    res.json({
      success: true,
      ticket: support.getTicket(req.params.id, userId)
    });
  } catch (error) {
    res.status(400).json({
      success: false,
...
app.post("/api/support/tickets/:id/messages", (req, res) => {
  try {
    ...
    const result = support.addUserMessage(
      req.params.id,
      userId,
      req.body.message
    );
...
app.get("/api/owner/support/tickets", (req, res) => {
  try {
    res.json({
      success: true,
      tickets: support.listAllTickets({
...
app.get("/api/owner/support/tickets/:id", (req, res) => {
  try {
    res.json({
      success: true,
      ticket: support.getTicket(req.params.id)
    });
  } catch (error) {
    res.status(400).json({
      success: false,
...
app.post("/api/owner/support/tickets/:id/reply", (req, res) => {
  try {
    const adminId = req.body.adminId || "owner";

    const result = support.replyToTicket(
      req.params.id,
      adminId,
      req.body.message
    );
...
app.patch("/api/owner/support/tickets/:id/status", (req, res) => {
  try {
    const ticket = support.changeTicketStatus(
      req.params.id,
      req.body.status
    );
