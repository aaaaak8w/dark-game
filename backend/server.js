...
console.log(`\\n[DARK GAME OTP] شماره: ${phone} | کد ورود: ${code}\\n`);
...
app.post("/api/auth/verify-otp", (req, res) => {
  ...
  console.log(`[DARK GAME VERIFY] phone=${phone} code=${code}`);
  ...
  if (saved.code !== code) {
    console.log(`[DARK GAME OTP ERROR] expected=${saved.code} received=${code}`);
    return res.status(400).json({
      success: false,
...
app.post("/api/me/mobile/request", auth, (req, res) => {
  ...
  console.log(
    `[DARK GAME MOBILE CHANGE OTP] ${newPhone}: ${code}`
  );
...
app.get("/api/owner/deposits", auth, (req, res) => {
  if (req.user.role !== "owner") {
    return res.status(403).json({
      success: false,
      message: "دسترسی فقط برای مالک است"
    });
  }

  const db = loadDatabase();

  const deposits = (db.topups || [])
    .map(t => {
      const user = (db.users || []).find(
...
app.get("/api/owner/transactions", auth, (req, res) => {
  if (req.user.role !== "owner") {
    return res.status(403).json({
      success: false,
      message: "دسترسی فقط برای مالک است"
    ...
    const notification = {
      ...
      userId:
        req.body.userId
          ...
          : null,
      title: String(
        req.body.title || "اعلان DARK GAME"
      ),
      text: String(
        req.body.text || ""
      ),
